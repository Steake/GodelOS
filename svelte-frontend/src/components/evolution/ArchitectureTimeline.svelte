<script>
  ...
  function generateMockTimelineData() {
    const now = Date.now();
    const timeSpan = getTimeSpanMs(timeRange);
    const startTime = now - timeSpan;
    
    const events = [];
    const eventTypeKeys = Object.keys(eventTypes);
    const impactKeys = Object.keys(impactLevels);
    ...
    for (let i = 0; i < numEvents; i++) {
      ...
      const impact = impactKeys[Math.floor(Math.random() * impactKeys.length)];
      ...
      events.push({
        ...
        metrics: generateEventMetrics(),
        related_events: [], // Will be populated later
        duration: Math.random() * 60 * 60 * 1000, // 0-1 hour
    ...
    events.forEach((event, i) => {
      if (Math.random() > 0.7 && i > 0) {
        const relatedIndex = Math.max(0, i - Math.floor(Math.random() * 3) - 1);
        event.related_events.push(events[relatedIndex].id);
      }
    });
  ...
  function generateEventTitle(eventType) {
    const titles = {
      architecture_change: [
        'Cognitive Engine Restructuring',
        'Memory System Redesign',
        'Inference Pipeline Update',
        'Module Interface Refactoring'
      ],
      capability_milestone: [
        'Advanced Reasoning Unlocked',
        'Multi-modal Understanding Achieved',
        'Learning Rate Optimization Completed',
        'Metacognitive Awareness Enhanced'
      ],
      performance_improvement: [
        'Query Response Time Reduced',
        'Memory Efficiency Increased',
        'Processing Throughput Enhanced',
        'Resource Utilization Optimized'
      ],
      integration_update: [
        'Knowledge Graph Integration',
        'External API Connection Enhanced',
        'Cross-Module Communication Improved',
      ...
      learning_breakthrough: [
        ...
        'Knowledge Consolidation Achieved'
      ],
      system_optimization: [
    ...
    const eventTitles = titles[eventType] || ['System Update'];
    return eventTitles[Math.floor(Math.random() * eventTitles.length)];
  }
  
  function generateEventDescription(eventType) {
    return `Significant ${eventType.replace('_', ' ')} event that affected system performance and capabilities.`;
  }
  ...
  function generateAffectedComponents() {
    ...
    return shuffled.slice(0, numComponents);
  }
  
  function generateEventMetrics() {
    return {
      performance_impact: (Math.random() - 0.5) * 0.4, // -20% to +20%
      stability_change: (Math.random() - 0.5) * 0.3, // -15% to +15%
      resource_usage_change: (Math.random() - 0.5) * 0.2, // -10% to +10%
      user_satisfaction_impact: Math.random() * 0.3 + 0.1 // +10% to +40%
    };
  }
