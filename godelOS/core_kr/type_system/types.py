...
class Type(ABC):
    ...
    @abstractmethod
    def is_subtype_of(
        self, other_type: "Type", type_system: "TypeSystemManager"
    ) -> bool:
        """
    ...
    def accept(self, visitor: "TypeVisitor") -> Any:
        """Accept a TypeVisitor and return its result."""
        pass
...
class FunctionType(Type):
    ...
    def is_subtype_of(
        ...
        for i in range(len(self._arg_types)):
            if not other_type._arg_types[i].is_subtype_of(
                self._arg_types[i], type_system
            ):
                return False
    ...
    def substitute_type_vars(self, bindings: Dict["TypeVariable", "Type"]) -> "Type":
        ...
        """
        new_arg_types = [
            arg_type.substitute_type_vars(bindings) for arg_type in self._arg_types
        ]
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, FunctionType):
            return False
        return (
            self._arg_types == other._arg_types
...
class ParametricTypeConstructor(ParametricType):
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, ParametricTypeConstructor):
            return False
        return self._name == other._name and self._type_params == other._type_params
    ...
    def accept(self, visitor: "TypeVisitor") -> Any:
        return visitor.visit_parametric_type(self)
...
class InstantiatedParametricType(ParametricType):
    ...
    def __init__(
        self, constructor: ParametricTypeConstructor, actual_type_args: List[Type]
    ):
        """
        ...
        if len(actual_type_args) != len(constructor.type_params):
            raise ValueError(
                f"Expected {len(constructor.type_params)} type arguments, got {len(actual_type_args)}"
            )
    ...
    def substitute_type_vars(self, bindings: Dict["TypeVariable", "Type"]) -> "Type":
        ...
        """
        new_type_args = [
            arg.substitute_type_vars(bindings) for arg in self._actual_type_args
        ]
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, InstantiatedParametricType):
            return False
        return (
            self._constructor == other._constructor
            and self._actual_type_args == other._actual_type_args
        )

    def __hash__(self) -> int:
        return hash(
            ("InstantiatedParametricType", self._constructor, self._actual_type_args)
        )
    ...
    def __repr__(self) -> str:
        return (
            f"InstantiatedParametricType({self._constructor}, {self._actual_type_args})"
        )
    ...
    def accept(self, visitor: "TypeVisitor") -> Any:
        return visitor.visit_instantiated_parametric_type(self)
