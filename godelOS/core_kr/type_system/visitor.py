...
from godelOS.core_kr.ast.nodes import (ApplicationNode, AST_Node, ASTVisitor,
                                       ConnectiveNode, ConstantNode,
                                       DefinitionNode, LambdaNode, ModalOpNode,
                                       QuantifierNode, VariableNode)
from godelOS.core_kr.type_system.environment import TypeEnvironment
from godelOS.core_kr.type_system.types import FunctionType, Type
...
class TypeVisitor(Generic[T]):
    """Base class for visitors over type objects."""

    def visit_atomic_type(self, type_obj: "AtomicType") -> T:
        raise NotImplementedError

    def visit_function_type(self, type_obj: "FunctionType") -> T:
        raise NotImplementedError

    def visit_parametric_type(self, type_obj: "ParametricType") -> T:
        raise NotImplementedError

    def visit_instantiated_parametric_type(
        self, type_obj: "InstantiatedParametricType"
    ) -> T:
        raise NotImplementedError

    def visit_type_variable(self, type_obj: "TypeVariable") -> T:
        raise NotImplementedError
...
class TypeInferenceVisitor(ASTVisitor[Tuple[Optional[Type], List[Error]]]):
    ...
    def visit_variable(self, node: VariableNode) -> Tuple[Optional[Type], List[Error]]:
        ...
        if var_type is None:
            ...
            if var_type is None:
                return None, [
                    Error(f"Cannot determine type for variable {node.name}", node)
                ]
        return var_type, []

    def visit_application(
        self, node: ApplicationNode
    ) -> Tuple[Optional[Type], List[Error]]:
        """
        ...
        if not isinstance(op_type, FunctionType):
            return None, [
                Error(f"Operator {node.operator} is not a function", node.operator)
            ]
    ...
    def visit_quantifier(
        ...
        if scope_type is None:
            return None, [
                Error("Cannot determine type for quantifier scope", node.scope)
            ]
    ...
    def visit_connective(
        self, node: ConnectiveNode
    ) -> Tuple[Optional[Type], List[Error]]:
        """
        ...
        for operand in node.operands:
            ...
            if operand_type is None:
                return None, [
                    Error("Cannot determine type for connective operand", operand)
                ]
    ...
    def visit_modal_op(self, node: ModalOpNode) -> Tuple[Optional[Type], List[Error]]:
        ...
        if prop_type is None:
            return None, [
                Error("Cannot determine type for modal proposition", node.proposition)
            ]
        ...
        if node.agent_or_world:
            ...
            if node.modal_operator in ["KNOWS", "BELIEVES"]:
                ...
                if agent_world_type is None:
                    return None, [
                        Error(
                            "Cannot determine type for modal agent/world",
                            node.agent_or_world,
                        )
                    ]
    ...
    def visit_definition(
        ...
        if body_type is None:
            return None, [
                Error(
                    "Cannot determine type for definition body",
                    node.definition_body_ast,
                )
            ]
...
class TypeCheckingVisitor(ASTVisitor[List[Error]]):
    ...
    def visit_constant(self, node: ConstantNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(node.type, self.expected_type):
            return [
                Error(
                    f"Constant {node.name} has type {node.type}, expected {self.expected_type}",
                    node,
                )
            ]
        return []
    ...
    def visit_variable(self, node: VariableNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(var_type, self.expected_type):
            return [
                Error(
                    f"Variable {node.name} has type {var_type}, expected {self.expected_type}",
                    node,
                )
            ]
    ...
    def visit_application(self, node: ApplicationNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(app_type, self.expected_type):
            return [
                Error(
                    f"Application has type {app_type}, expected {self.expected_type}",
                    node,
                )
            ]
    ...
    def visit_quantifier(self, node: QuantifierNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(quant_type, self.expected_type):
            return [
                Error(
                    f"Quantifier has type {quant_type}, expected {self.expected_type}",
                    node,
                )
            ]
    ...
    def visit_connective(self, node: ConnectiveNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(conn_type, self.expected_type):
            return [
                Error(
                    f"Connective has type {conn_type}, expected {self.expected_type}",
                    node,
                )
            ]
    ...
    def visit_modal_op(self, node: ModalOpNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(modal_type, self.expected_type):
            return [
                Error(
                    f"Modal operator has type {modal_type}, expected {self.expected_type}",
                    node,
                )
            ]
    ...
    def visit_lambda(self, node: LambdaNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(lambda_type, self.expected_type):
            return [
                Error(
                    f"Lambda has type {lambda_type}, expected {self.expected_type}",
                    node,
                )
            ]
    ...
    def visit_definition(self, node: DefinitionNode) -> List[Error]:
        ...
        if not self.type_system.is_subtype(def_type, self.expected_type):
            return [
                Error(
                    f"Definition has type {def_type}, expected {self.expected_type}",
                    node,
                )
            ]
