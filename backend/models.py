...
class KnowledgeItem(BaseModel):
    ...
    created_at: float = Field(..., description="Timestamp when knowledge was added")
...
class ProcessState(BaseModel):
    ...
    started_at: float = Field(..., description="Timestamp when process started")
...
class AttentionFocus(BaseModel):
    ...
    duration: float = Field(..., description="How long this item has been in focus (seconds)")
...
class WorkingMemoryItem(BaseModel):
    ...
    created_at: float = Field(..., description="Timestamp when item was created")
    last_accessed: float = Field(..., description="Timestamp when item was last accessed")
    access_count: int = Field(..., description="Number of times item has been accessed")
...
class MetacognitiveState(BaseModel):
    ...
    cognitive_load: float = Field(..., ge=0.0, le=1.0, description="Current cognitive load")
    learning_rate: float = Field(..., ge=0.0, le=1.0, description="Current learning effectiveness")
    adaptation_level: float = Field(..., ge=0.0, le=1.0, description="System adaptation level")
    introspection_depth: int = Field(..., description="Current depth of introspective analysis")
...
class ManifestConsciousness(BaseModel):
    ...
    integration_level: float = Field(..., ge=0.0, le=1.0, description="Integration across cognitive systems")
