...
import {recurrence,selectAttention,commitAttention} from './attention12.mjs';
import {applyVerifiedProgress} from './verification12.mjs';
import {ensureLiving,livingContext,livingBids,finishLiving,LIVING_CONTRACT} from './living13.mjs';

export const MIND_VERSION = '13.0.0';
...
export function ensureMind(state) {
  if(state.mind){if(!['8.0.0','10.0.0','12.0.0',MIND_VERSION].includes(state.mind.schema))throw Error('Unsupported mind schema; existing state preserved');const changed=state.mind.schema!==MIND_VERSION;state.mind.schema=MIND_VERSION;state.mind.personality||={warmth:.55,playfulness:.4,directness:.65,reflectiveness:.6,history:[]};ensureLiving(state);return changed;}
  state.mind = {
  ...
  ensureLiving(state);return true;
}

function appraise(state, input, ablation) {
  const mind = state.mind, prior = state.affect || {};
  const development=mind.living?.events.filter(e=>e.tick===mind.tick).at(-1);
  const outcome = input.outcome&&input.outcome!=='none'?input.outcome:development?.kind==='opinion_revised'?'contradiction':development?.kind==='association'?'novel':'none';
...
export function prepareMind(stateInput, input = {}, {ablation = 'none'} = {}) {
  ...
  const lifeContext=livingContext(state,tick,{ablation});
  const bids = [...candidates(state, input, tick),...livingBids(lifeContext,affect)].map(c => {
    const legacyRepeats = m.focus_history.slice(-8).filter(h => h.id === c.id || (h.title && overlap(c.title,h.title)>.65)).length;
    const recurrenceState=recurrence(m,c,legacyRepeats),repeats=Math.min(8,recurrenceState.stagnant_visits);
    ...
    return {...c,score,recurrence:recurrenceState,policy:m.experimental_policy?'experimental':'baseline',components:m.experimental_policy?{experimental_policy:score}:{salience:.35*c.salience,need:.4*c.need,affect:m.policy.affect_gain*affectTerm,habituation:-m.policy.habituation*repeats,monitor}};
  }).sort((a,b) => b.score-a.score || a.id.localeCompare(b.id));
  // Human contact enters the workspace promptly; idle attention is competitive.
  const selection=selectAttention(m,bids,input);let focus=selection.focus;
  ...
  const workspace = {tick, focus, selection_reason:selection.reason,attention_policy:'recurrence-1',competitors: ablation === 'workspace' ? [] : bids.filter(c=>c.id!==focus.id).slice(0,5), retrieved, ablation};
  workspace.living=lifeContext;
  ...
  return {state, input, tick, mode, workspace, predictionError, ablation,lifeContext};
}

export const MIND_CONTRACT = {
  living_update:LIVING_CONTRACT,
  ...
  intention_updates: [{id:'existing ID or null',goal:'chosen intention',steps:['observable step'],priority:.5,status:'active|blocked|completed|abandoned',evidence:'why this intention or status is warranted',advance:false,artifact:'Only when a verification_contract is supplied: budget_plan uses {decisions:[option IDs]}; attention_policy uses {program:bounded AST}. Otherwise omit. Prose progress remains unverified; only the registered adapter advances steps.'}],
...
export function finishMind(prepared, parsed) {
  ...
  const suppliedReferences=list(update.memory_references).map(id=>text(id,100));
  const opinionReferences=suppliedReferences.filter(id=>prepared.lifeContext.opinions.some(o=>o.id===id));
  const memoryReferences=suppliedReferences.filter(id=>!opinionReferences.includes(id));
  if(memoryReferences.some(id=>!workspace.retrieved.some(m=>m.id===id)&&!prepared.lifeContext.associative_seeds.some(s=>s.id===id&&s.kind==='episode')))throw new Error('Memory reference was not retrieved in this episode');
  ...
  for (const proposed of list(update.intention_updates,3)) {
    ...
    if (!current) {
      ...
      if(m.intentions.filter(i=>['active','blocked'].includes(i.status)).length>=60)throw new Error('Active intention capacity reached; resolve or abandon an existing intention before adding another');
    ...
    if (proposed.advance === true || proposed.status==='completed') current.progress_claim={tick,reason,requested_status:proposed.status||'active',provenance:'unverified_model_claim'};
    if (['active','blocked','abandoned'].includes(proposed.status)) current.status=proposed.status;
    if(proposed.artifact){try{applyVerifiedProgress(current,proposed.artifact,tick);}catch(e){current.verification_error={tick,message:e.message};}}
  ...
  if (text(update.social_initiative?.message)&&!text(update.living_update?.invitation?.message)&&tick-m.living.last_social_tick>=6) {m.outbox.push({id:uid('invitation'),message:text(update.social_initiative.message),reason:text(update.social_initiative.reason),tick,status:'awaiting_operator',provenance:'model_initiated'});m.living.last_social_tick=tick;}
  ...
  const livingWarnings=finishLiving(prepared.state,state,parsed,tick,prepared.lifeContext);
  ...
  m.last_trace.broadcast={selected_by:workspace.selection_reason||'competitive_attention',inputs:[workspace.focus,...workspace.competitors].map(c=>({id:c.id,kind:c.kind,title:c.title,score:c.score,components:c.components,policy:c.policy,recurrence:c.recurrence})),appraisal:prepared.state.affect,expression:expressionPolicy(prepared.state,prepared.ablation),stance_changes:(state.beliefs||[]).flatMap(b=>(b.history||[]).filter(h=>h.episode===state.episode_count).map(h=>({position_id:b.position_id,proposition:b.proposition,...h})))};
  m.last_trace.living_warnings=livingWarnings;
  m.last_trace.opinion_references=opinionReferences;
  m.last_trace.reference_type_repairs=opinionReferences.map(id=>({id,from:'memory_reference',to:'recorded_opinion',reason:'ID resolves to an opinion actually supplied in context'}));
  m.intentions=[...m.intentions.filter(i=>['active','blocked'].includes(i.status)),...m.intentions.filter(i=>!['active','blocked'].includes(i.status)).slice(-60)];
  for (const [key,limit] of Object.entries({thoughts:40,predictions:100,prediction_errors:100,focus_history:100,revisions:40,outbox:40})) m[key]=m[key].slice(-limit);
