...
function structure(source){
 ...
 for(let i=0;i<source.length;i++){
  ...
  if(ch==='"'){inString=true;start=i;}
   ...
   if(stack.pop()!==(ch==='}'?'{':'['))return {invalid:true,rootEnd};scopes.pop();
...
export function parseMindCompletion(completion,mode){
  try{return {parsed:parseCompletion(completion.text,mode),repair:null};}catch(original){
    ...
    // JSON requires escaped control characters inside strings. Escaping the
    // literal character preserves its decoded value, including line breaks.
    let escapedSource='',quoted=false,backslash=false;
    for(const ch of source){
      if(quoted&&ch.charCodeAt(0)<32){escapedSource+=JSON.stringify(ch).slice(1,-1);continue;}
      escapedSource+=ch;
      if(quoted){if(backslash)backslash=false;else if(ch==='\\')backslash=true;else if(ch==='"')quoted=false;}
      else if(ch==='"')quoted=true;
    }
    if(escapedSource!==source){
      const recovered=parseMindCompletion({...completion,text:escapedSource},mode);
      return {parsed:recovered.parsed,repair:{kind:'escape_literal_string_controls',raw_sha256:sha256(completion.text),derived_sha256:sha256(escapedSource),following_repair:recovered.repair}};
    }
    const boundary=structure(source);
    if(boundary.rootEnd>=0&&/^\s*}(?:\s*})?\s*$/.test(source.slice(boundary.rootEnd+1))){
      const candidate=source.slice(0,boundary.rootEnd+1),shape=structure(candidate);
      if(!shape.invalid&&!shape.duplicateKeys)try{return {parsed:parseCompletion(candidate,mode),repair:{kind:'remove_redundant_trailing_closures',removed:source.slice(boundary.rootEnd+1),raw_sha256:sha256(completion.text),derived_sha256:sha256(candidate)}};}catch{}
    }
