...
import {enrichTask,taskUtility} from './task-adapters12.mjs';
...
export function authorSuite({seed=12031,clusters=8,difficulty=1}={}){
 ...
 for(let c=0;c<clusters;c++)for(let variant=0;variant<2;variant++){
  ...
  enrichTask(task);task.expected=oracle(task);tasks.push(task);
 }
 const content={schema:'transfer-suite-12.2',author:'deterministic engine with four distinct task adapters, separate from LM under test',seed,clusters,difficulty,gate:TRANSFER_GATE,tasks};
...
export function oracle(task){
 ...
 return task.options.map(o=>({...o,utility:taskUtility(task,o,a,b)})).filter(o=>Number.isFinite(o.utility)).sort((x,y)=>y.utility-x.utility||Number(x.id.split('_')[1])-Number(y.id.split('_')[1]))[0]?.id||'defer';
}
...
export function analyseTransfer(rows,plan,{calibrated=false}={}){
 ...
 limitations:['Four engineered adapters cover budget allocation, dependency recovery, executable patch tests and deadline schedules; diversity within each remains limited.','Scenario-cluster intervals do not establish generalisation beyond these four generators.','Retrieved state effects do not demonstrate changes to model weights or subjective consciousness.','Invalid or failed provider outputs count as zero utility; no selective deletion.']};
}
